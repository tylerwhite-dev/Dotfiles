...
`setup.sh` resolves the script and repository roots, loads `logic/load.sh`, and
calls `setup_run`.
...
| Path | Responsibility |
| --- | --- |
| `setup.sh` | Thin executable entry point. |
...
Use `bash -n` for syntax-only checks. Do not run `setup.sh` without an explicit
